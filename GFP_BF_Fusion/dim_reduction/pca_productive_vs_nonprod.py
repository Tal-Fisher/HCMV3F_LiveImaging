#!/usr/bin/env python3
"""
pca_productive_vs_nonprod.py

PCA on all GFP-expressing cells (productive + non-productive), using
*_all.npz embeddings. Colors: productive (turns red) vs non-productive.

No half-movie filter — we want all cells regardless of when GFP appeared.

Feature sets: BF (256), GFP (256), BF+GFP (512).
Output: dim_reduction/pca_prod_vs_nonprod_all.png  (1x3 panel)
"""
...
COLOR_PROD    = '#e06c75'   # red-ish — turns red
COLOR_NONPROD = '#98c379'   # green-ish — never turns red
ALPHA = 0.55
S     = 18
...
for ds in DATASETS:
    ...
    if not gfp_f.exists() or not bf_f.exists():
        print(f'  {ds}: _all.npz not yet available — skipping '
              f'(run submit_gfp_all_{ds.lower()}.sh and submit_bf_all_{ds.lower()}.sh first)')
        continue
...
# Inner join: cells present in BOTH GFP and BF embedding files
...
for idx, (title, X) in enumerate(feature_sets):
    ...
    make_ax(axes[idx], Z, y, evr, title)

fig_all.suptitle('PCA — Productive vs Non-Productive Cells at GFP Onset',
                 fontsize=13, fontweight='bold', y=1.02)
fig_all.tight_layout()
fig_all.savefig(OUT_DIR / 'pca_prod_vs_nonprod_all.png', dpi=300, bbox_inches='tight')
plt.close(fig_all)
print('saved pca_prod_vs_nonprod_all.png')
