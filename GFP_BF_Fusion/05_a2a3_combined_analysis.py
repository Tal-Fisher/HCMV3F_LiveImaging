#!/usr/bin/env python3
"""
05_a2a3_combined_analysis.py

Combined A2+A3 GFP+BF classification.
No half-movie filter — all cells with finite delay_blue_to_red are included.
...
Outputs
-------
  results/a2a3_combined_metrics.csv   -- 18 rows (3 datasets × 2 model types × 3 feature sets)
  results/a2a3_combined_oof.csv       -- OOF predictions for all conditions
  figures/a2a3_combined_roc.png       -- 3×2 ROC grid
  figures/a2a3_combined_auc_bars.png  -- grouped AUC bar chart
"""
...
for ds_list, ds_label in CONDITIONS:
    ...
    for X_in, feat in glm_runs:
        ...
        oof_store[(ds_label, 'GLM', feat)] = (oof, y.copy())
    ...
    for X_in, feat in net_runs:
        ...
        all_records.append(m)
        oof_store[(ds_label, 'Network', feat)] = (oof, y.copy())

# Save results
col_order = ['dataset', 'model_type', 'features', 'n', 'n_fast',
             'auc', 'ap', 'sens', 'spec', 'bal_acc', 'mcc']
results = pd.DataFrame(all_records)[col_order]
results.to_csv(RESULTS_DIR / 'a2a3_combined_metrics.csv', index=False)
print(f'\nSaved results/a2a3_combined_metrics.csv', flush=True)
...
pd.DataFrame(oof_rows).to_csv(RESULTS_DIR / 'a2a3_combined_oof.csv', index=False)
print('Saved results/a2a3_combined_oof.csv', flush=True)
...
fig.suptitle(
    ...
    f'5-fold OOF  |  no half-movie filter',
    fontsize=11, fontweight='bold', y=1.01)
plt.tight_layout()
fig.savefig(str(FIGURES_DIR / 'a2a3_combined_roc.png'), dpi=150, bbox_inches='tight')
plt.close(fig)
print('Saved figures/a2a3_combined_roc.png', flush=True)
...
plt.tight_layout()
fig.savefig(str(FIGURES_DIR / 'a2a3_combined_auc_bars.png'), dpi=150, bbox_inches='tight')
plt.close(fig)
print('Saved figures/a2a3_combined_auc_bars.png', flush=True)
