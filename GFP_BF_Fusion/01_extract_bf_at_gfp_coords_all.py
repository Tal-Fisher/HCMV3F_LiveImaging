#!/usr/bin/env python3
"""
Extract 256-dim Cellpose SAM neck embeddings from the BF channel at the
GFP onset frame for ALL GFP-expressing cells (productive + non-productive).

Usage:
  python 01_extract_bf_at_gfp_coords_all.py --dataset A2
  python 01_extract_bf_at_gfp_coords_all.py --dataset A3
...
Output:
  GFP_BF_Fusion/embeddings/{DATASET}_bf_at_gfp_onset_all.npz
  GFP_BF_Fusion/embeddings/{DATASET}_bf_at_gfp_onset_all.csv
"""
...
npz_path = OUT_DIR / f'{DATASET}_bf_at_gfp_onset_all.npz'
np.savez(str(npz_path), track_ids=track_ids_arr, embeddings=embeddings_arr)
print(f'Saved: {npz_path}')

csv_path = OUT_DIR / f'{DATASET}_bf_at_gfp_onset_all.csv'
...
for ax, (tid, (crop, frame)) in zip(axes, sample_crops.items()):
    ...
    ax.axis('off')
fig.suptitle(f'BF crops at GFP onset (all cells) — {DATASET}', fontsize=11)
plt.tight_layout()
png_path = FIG_DIR / f'sample_crops_bf_all_{DATASET}.png'
