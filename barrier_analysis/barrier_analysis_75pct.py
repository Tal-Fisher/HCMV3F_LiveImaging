"""
barrier_analysis_75pct.py — Barrier analysis: 75% productive-cell crossing

Barrier definition:
  A feature value T is the "75% barrier" if ≥75% of productive cells crossed T
  at some point between GFP onset and red onset.

  Direction per feature:
    "max" — cell must REACH AT LEAST T
             T = 25th percentile of (max value per productive cell before red onset)
    "min" — cell must DROP TO AT MOST T
             T = 75th percentile of (min value per productive cell before red onset)

STRICTNESS = 0.25 → 75% of productive cells pass the barrier.
Compare to 85% version (barrier_analysis.py) and 100% version (barrier_analysis_100pct.py).
"""
...
STRICTNESS = 0.25  # 25th/75th percentile → 75% pass
...
# Compute 75% barriers
# ══════════════════════════════════════════════════════════════════════════════
print("Computing 75% barriers ...", flush=True)
...
for col, label, direction in FEAT_SPECS:
    ...
    # 75% threshold: 25th pct of peaks (max) or 75th pct of troughs (min)
    if direction == "max":
        T = float(np.nanpercentile(early_ext["early_ext"], STRICTNESS * 100))
        ...
        T = float(np.nanpercentile(early_ext["early_ext"], (1 - STRICTNESS) * 100))
        pct_prod    = (early_ext["early_ext"] <= T).mean() * 100
...
barrier_df.to_csv(OUT_DIR / "barrier_thresholds_75pct.csv", index=False)
print("  Saved barrier_thresholds_75pct.csv", flush=True)
...
fig.suptitle(
    "75% barrier analysis — feature crossing rates\n"
    "Threshold = value reached by ≥75% of productive cells before red onset\n"
...
ax.barh(y + h/2, valid["pct_prod_cross"],  h, color="#27ae60", alpha=0.85,
        label="Productive: reached 75% threshold before red onset")
...
ax.set_xlim(0, 115)
ax.axvline(75, color="#27ae60", lw=0.8, linestyle="--", alpha=0.6, label="75% target")
...
plt.tight_layout()
fig.savefig(OUT_DIR / "barrier_summary_75pct.png", dpi=150, bbox_inches="tight")
plt.close()
print("  Saved barrier_summary_75pct.png", flush=True)
...
fig.suptitle(
    "Feature trajectories from GFP onset + 75% barrier thresholds\n"
...
plt.tight_layout()
fig.savefig(OUT_DIR / "barrier_trajectories_75pct.png", dpi=150, bbox_inches="tight")
plt.close()
print("  Saved barrier_trajectories_75pct.png", flush=True)
...
top4 = (barrier_df.dropna(subset=["gap"])
        .loc[barrier_df["feature"] != "Mean.ch1"]  # drop BFP cytoplasm
        .nlargest(3, "gap"))

fig, axes = plt.subplots(1, 3, figsize=(15, 5))
fig.suptitle(
    "When do productive cells first cross the 75% barrier?\n"
    "(x = hours before red onset; dotted lines = mean red onset time per group)",
    fontsize=11, fontweight="bold"
)
...
for ax, (_, row) in zip(axes.flat, top4.iterrows()):
    ...
    pct_cross = len(cell_crossing[key].dropna(subset=["cross_h_before_red"])) / prod_cells * 100
    ...
    for grp, col in [("early", GROUP_COLORS["early"]),
        ...
        # dotted vertical line: mean crossing time for this group
        grp_mean = df_c.loc[df_c["group"] == grp, "cross_h_before_red"].mean()
        if np.isfinite(grp_mean):
            ax.axvline(grp_mean, color=col, lw=1.2, linestyle=":",
                       alpha=0.85, label=f"{grp} mean {grp_mean:.0f}h")

    ax.set_xlabel("Hours before red onset that 75% barrier was crossed", fontsize=8)
...
plt.tight_layout()
fig.savefig(OUT_DIR / "barrier_crossing_75pct.png", dpi=150, bbox_inches="tight")
plt.close()
print("  Saved barrier_crossing_75pct.png", flush=True)
