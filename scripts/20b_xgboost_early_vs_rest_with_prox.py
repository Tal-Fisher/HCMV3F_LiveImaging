"""
20b_xgboost_early_vs_rest_with_prox.py

XGBoost binary classification: early vs medium+late — 45 features + proximity.
Tests whether adding dist_nearest and n_within_100 back recovers AUC.
"""
...
PROX_DIR    = BASE / "proximity_analysis" / "results"
FIG_DIR     = BASE / "figures" / "combined"
RESULTS_DIR = BASE / "results" / "xgboost_early_with_prox"
...
feat_cols += ["dist_nearest", "n_within_100"]
print(f"Features: {len(feat_cols)}  (45 extended + 2 proximity)")

cat_df  = pd.read_csv(EXPORT_DIR / "category_df.csv")
prox_df = pd.read_csv(PROX_DIR / "proximity_features.csv")

df = ext_df.merge(cat_df[["Track.ID", "category"]], on="Track.ID", how="left")
df = df.merge(prox_df[["Track_ID", "dist_nearest", "n_within_100"]],
...
fig.suptitle(
    ...
    f"{N_FOLDS}-fold nested CV  |  scale_pos_weight={spw:.1f}  |  threshold={THRESHOLD}  |  {len(feat_cols)} features (45 ext + 2 prox)",
...
plt.tight_layout()
out = FIG_DIR / "xgboost_early_vs_rest_with_prox.png"
...
plt.tight_layout()
bees_out = FIG_DIR / "xgboost_early_vs_rest_with_prox_shap_beeswarm.png"
