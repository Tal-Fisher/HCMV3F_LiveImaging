#!/usr/bin/env python3
"""
Redo regression (delay_blue_to_red) + classification (early vs med+late)
using only the top 20 embedding dims selected by average rank across both
tasks from the 116-dim analysis.

Top 20 dims (by avg |beta| rank, regression + classification):
  [212, 148, 127, 204, 237, 200, 241, 198, 77, 60, 66, 247, 11, 163, 223, 78, 205, 44, 190, 249]

Output:
  results/top20_metrics.csv
  results/top20_regression_betas.csv
  results/top20_classify_betas.csv
...
X_top = embeddings[:, TOP20_DIMS]   # (291, 20)
print(f'  Feature matrix: {X_top.shape}  (dims: {TOP20_DIMS})')
...
print(f'  Regression (blue→red): n={len(y_reg)}  mean={y_reg.mean():.0f}  std={y_reg.std():.0f} min')
...
# ══════════════════════════════════════════════════════════════════════════
print('\n═══ REGRESSION (delay_blue_to_red, top-20 dims) ═══', flush=True)
...
cv_r2_r = r2_score(y_reg, oof_reg)
cv_r_r  = pearsonr(y_reg, oof_reg)[0]
cv_mae  = mean_absolute_error(y_reg, oof_reg)
print(f'  5-fold CV:  R²={cv_r2_r:.3f}  r={cv_r_r:.3f}  MAE={cv_mae:.1f} min')
print(f'  Reference (116-dim): r=0.271')

sc_r = StandardScaler()
en_f = ElasticNetCV(l1_ratio=[0.5, 0.9, 1.0], alphas=np.logspace(-2, 3, 20),
                    cv=5, max_iter=5000, n_jobs=-1, random_state=SEED)
en_f.fit(sc_r.fit_transform(X_reg), y_reg)
betas_r = en_f.coef_
print(f'  Full-data: alpha={en_f.alpha_:.4f}  l1={en_f.l1_ratio_:.2f}  '
      f'non-zero: {(betas_r!=0).sum()}/20')
print(f'  Betas: { {f"dim_{TOP20_DIMS[i]}": round(float(b),2) for i,b in enumerate(betas_r) if b!=0} }')

pd.DataFrame([dict(task='regression_blue_to_red', n_dims=20, n=len(y_reg),
                   cv_r2=round(cv_r2_r,4), cv_r=round(cv_r_r,4), cv_mae=round(cv_mae,1),
                   n_nonzero=int((betas_r!=0).sum()),
                   ref_116dim_r=0.271)
             ]).to_csv(str(RESULTS_DIR/'top20_regression_metrics.csv'), index=False)

pd.DataFrame({'orig_dim': TOP20_DIMS, 'beta': betas_r,
              'abs_beta': np.abs(betas_r)}
             ).sort_values('abs_beta', ascending=False
             ).to_csv(str(RESULTS_DIR/'top20_regression_betas.csv'), index=False)

plot_betas(betas_r, TOP20_DIMS, 'Regression betas: delay_blue_to_red (top-20 dims)',
           FIGURES_DIR/'top20_regression_betas.png')
...
ax.set_title(f'delay_blue_to_red — top-20 dims\nCV R²={cv_r2_r:.3f}  r={cv_r_r:.3f}  (n={len(y_reg)})')
ax.legend(title='Tertile', fontsize=8)
plt.tight_layout()
fig.savefig(str(FIGURES_DIR/'top20_scatter_regression.png'), dpi=150, bbox_inches='tight')
plt.close(fig)
print(f'  Saved figures/top20_regression_betas.png + top20_scatter_regression.png')

# ══════════════════════════════════════════════════════════════════════════
# CLASSIFICATION — early vs med+late
# ══════════════════════════════════════════════════════════════════════════
print('\n═══ CLASSIFICATION (early vs med+late, top-20 dims) ═══', flush=True)
...
print(f'  Reference (116-dim): AUC=0.668  |  tabular: AUC=0.674')

sc_c = StandardScaler()
lr_f = LogisticRegressionCV(**{**LR_CV,
           'cv': StratifiedKFold(n_splits=5, shuffle=True, random_state=SEED)})
lr_f.fit(sc_c.fit_transform(X_cls), y_cls)
betas_c = lr_f.coef_[0]
print(f'  Full-data: C={lr_f.C_[0]:.4f}  l1={lr_f.l1_ratio_[0]:.2f}  '
      f'non-zero: {(betas_c!=0).sum()}/20')
print(f'  Betas: { {f"dim_{TOP20_DIMS[i]}": round(float(b),3) for i,b in enumerate(betas_c) if b!=0} }')

pd.DataFrame([dict(task='classify_early_vs_rest', n_dims=20, n=len(y_cls),
                   n_early=int(y_cls.sum()), auc=round(auc,3),
                   sens=round(sens,3), spec=round(spec,3), bal_acc=round(bal,3),
                   n_nonzero=int((betas_c!=0).sum()),
                   ref_116dim_auc=0.668, ref_tabular_auc=0.674)
             ]).to_csv(str(RESULTS_DIR/'top20_classify_metrics.csv'), index=False)

pd.DataFrame({'orig_dim': TOP20_DIMS, 'beta': betas_c,
              'abs_beta': np.abs(betas_c)}
             ).sort_values('abs_beta', ascending=False
             ).to_csv(str(RESULTS_DIR/'top20_classify_betas.csv'), index=False)

plot_betas(betas_c, TOP20_DIMS, 'Classification betas: early vs med+late (top-20 dims)',
...
ax.axhline(0.668, color='#4CAF50', lw=1, ls=':', label='116-dim subset AUC=0.668')
ax.axhline(0.674, color='#2196F3', lw=1, ls=':', label='Tabular (A2) AUC=0.674')
ax.set_xlabel('1 − Specificity'); ax.set_ylabel('Sensitivity')
ax.set_title(f'Early vs Med+Late — top-20 embedding dims\n(n={len(y_cls)}, {y_cls.sum()} early)')
ax.legend(fontsize=8, loc='lower right')
plt.tight_layout()
fig.savefig(str(FIGURES_DIR/'top20_roc.png'), dpi=150, bbox_inches='tight')
plt.close(fig)
print(f'  Saved figures/top20_classify_betas.png + top20_roc.png')

# ── Combined summary ───────────────────────────────────────────────────────
print('\n── Full progression summary ──')
print(f'  {"Dims":<8}  {"Regr r":>8}  {"Classif AUC":>12}')
print(f'  {"256 raw":<8}  {0.277:>8.3f}  {0.637:>12.3f}')
print(f'  {"116":<8}  {0.271:>8.3f}  {0.668:>12.3f}')
print(f'  {"20":<8}  {cv_r_r:>8.3f}  {auc:>12.3f}')
print(f'  {"Tabular":<8}  {0.359:>8.3f}  {0.674:>12.3f}  (reference)')
