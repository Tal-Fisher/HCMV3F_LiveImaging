"""
04_run_forecaster_allwindows_h010.py
TabICLForecaster: predict mCherry trajectory H=10 frames ahead.
All windows included (pre-onset, crossing, post-onset).
No threshold, no normalization, pure trajectory prediction.
See approach_allwindows_h010.txt for full documentation.
"""
...
MIN_PER_FRAME = 15
RANDOM_STATE  = 42
N_PLOT        = 100   # windows for the PDF
...
# ── Same 75/25 split as all other analyses ────────────────────────────────────
...
# ── Build windows — NO remaining filter ──────────────────────────────────────
print("\nBuilding windows (all, no onset filter) ...", flush=True)
windows = []
for i, tid in enumerate(meta.index):
    ...
    for w_start in range(0, n_frames - CONTEXT_LEN, STRIDE):
        ...
        if n_future < H:
            continue   # not enough future frames for ground truth

        ts_ctx = pd.date_range(start="2000-01-01", periods=CONTEXT_LEN,
                               freq=f"{MIN_PER_FRAME}min")
        t_ctx_end  = t_arr[w_end - 1]
        remaining  = red_min - t_ctx_end   # can be negative (post-onset)
...
for g in ["early", "medium", "late"]:
    n = sum(1 for w in windows if w["group"] == g)
    print(f"    {g:8s}: {n:,}", flush=True)
...
with open(OUT_DIR / "tabicl_forecaster_allwindows.pkl", "wb") as f:
    pickle.dump(forecaster, f)
print("Saved tabicl_forecaster_allwindows.pkl", flush=True)
...
for w in windows:
    ...
    if iid not in pred_grp:
        continue
    pred_traj = np.array(pred_grp[iid])   # (H,)
    act_traj  = w["Y_future"]              # (H,)
    traj_dict[iid] = pred_traj

    mae_w  = mean_absolute_error(act_traj, pred_traj)
    naive  = np.full(H, w["Y_ctx"][-1])   # persistence baseline
...
np.savez_compressed(OUT_DIR / "trajectories_allwindows_h010.npz", **traj_dict)
print("Saved trajectories_allwindows_h010.npz", flush=True)

results_df = pd.DataFrame(records)
results_df.to_csv(OUT_DIR / "predictions_allwindows_h010.csv", index=False)
print(f"Saved predictions_allwindows_h010.csv ({len(results_df):,} rows)", flush=True)
...
pd.DataFrame(eval_rows).to_csv(OUT_DIR / "eval_allwindows_h010.csv", index=False)
print("Saved eval_allwindows_h010.csv", flush=True)

# ── PDF: 100 random windows ───────────────────────────────────────────────────
print(f"\nGenerating PDF of {N_PLOT} random windows ...", flush=True)
...
ts_raw_sub = ts[["Track.ID","T_min","abs_gfp_onset_min"]].copy()
cell_tmin2 = cell_tmin

PER_PAGE = 20
n_cols, n_rows = 5, 4
n_pages = int(np.ceil(len(chosen) / PER_PAGE))

pdf_path = OUT_DIR / "windows_allwindows_h010_100examples.pdf"
with PdfPages(pdf_path) as pdf:
    for page in range(n_pages):
        ...
        fig.suptitle(
            ...
            "(grey=context, solid=predicted, dashed=actual future, red▲=onset)",
        ...
        for ax_idx, ax in enumerate(axes.flat):
            ...
            # prepend last context point so lines connect without a gap
            ...
            # mark red onset if it falls within the plot
            rem_h = w["remaining_min"] / 60
            ctx_len_h = CONTEXT_LEN * MIN_PER_FRAME / 60
            total_h   = (CONTEXT_LEN + H) * MIN_PER_FRAME / 60
            onset_in_plot = ctx_len_h - rem_h   # negative = onset before context end
            # in plot coords: onset happens at (ctx_len_h - rem_h) from context start
            # = CONTEXT_LEN frames - remaining frames (in time units)
            t_arr  = cell_tmin2.get(tid, np.array([]))
            delay  = meta.loc[tid, "delay_green_to_red"]
            gfp_min = w.get("gfp_min",
                ts_raw_sub[ts_raw_sub["Track.ID"]==tid]["abs_gfp_onset_min"].iloc[0]
                if tid in ts_raw_sub["Track.ID"].values else 0)
            red_min = gfp_min + delay
