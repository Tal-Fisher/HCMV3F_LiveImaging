"""
05_run_forecaster_normalized_h010.py
TabICLForecaster: predict mCherry H=10 frames ahead, all windows.
mCherry z-scored per context window (mean/std of the 16 context frames).
Covariates provided for context frames only. Predictions denormalized back to original scale.
"""
...
for i, tid in enumerate(meta.index):
    ...
    for w_start in range(0, n_frames - CONTEXT_LEN, STRIDE):
        ...
        windows.append({
            ...
            "Y_ctx":         Y_ctx,
            "Y_ctx_norm":    (Y_ctx - ctx_mean) / ctx_std,
...
# ── Build context_df (normalized target + covariates) ────────────────────────
...
for w in windows:
    for t_idx in range(CONTEXT_LEN):
        row = {"item_id":   w["item_id"],
               ...
               "target":    float(w["Y_ctx_norm"][t_idx])}
...
with open(OUT_DIR / "tabicl_forecaster_normalized_h010.pkl", "wb") as f:
    pickle.dump(forecaster, f)
print("Saved tabicl_forecaster_normalized_h010.pkl", flush=True)
...
# ── Collect results — denormalize to original scale ───────────────────────────
traj_dict = {}
records   = []
for w in windows:
    iid = w["item_id"]
    if iid not in pred_grp:
        continue
    pred_norm = np.array(pred_grp[iid])
    pred_traj = pred_norm * w["ctx_std"] + w["ctx_mean"]
    act_traj  = w["Y_future"]
...
np.savez_compressed(OUT_DIR / "trajectories_normalized_h010.npz", **traj_dict)
print("Saved trajectories_normalized_h010.npz", flush=True)

results_df = pd.DataFrame(records)
results_df.to_csv(OUT_DIR / "predictions_normalized_h010.csv", index=False)
print(f"Saved predictions_normalized_h010.csv ({len(results_df):,} rows)", flush=True)
...
pd.DataFrame(eval_rows).to_csv(OUT_DIR / "eval_normalized_h010.csv", index=False)
print("Saved eval_normalized_h010.csv", flush=True)
