"""
06_run_forecaster_nomcherry_h010.py
TabICLForecaster: predict mCherry H=10 ahead using ONLY the 9 other channels.
mCherry is NOT provided as context — target column is set to global mean (constant),
forcing the model to rely entirely on GFP, BFP, morphology, and brightfield covariates.

Windows: pre-onset + crossing only (remaining_min > -H*15 min).
No normalization (TabICL normalizes internally).
"""
...
RANDOM_STATE  = 42
N_PLOT        = 100
WINDOW_FILTER = -H * MIN_PER_FRAME   # exclude deeply post-onset (remaining_min > -150)
...
print(f"  Test cells: {len(test_cells)}  Train cells: {len(train_cells)}", flush=True)

# ── Compute global mean mCherry from training cells ──────────────────────────
train_indices = [i for i, tid in enumerate(meta.index) if tid in train_cells]
global_mean_mcherry = float(np.nanmean(Y_tensor[train_indices]))
print(f"  Global mean mCherry (train): {global_mean_mcherry:.4f}", flush=True)

# ── Build windows (pre-onset + crossing only) ─────────────────────────────────
print(f"\nBuilding windows (remaining_min > {WINDOW_FILTER}) ...", flush=True)
windows = []
for i, tid in enumerate(meta.index):
    ...
    for w_start in range(0, n_frames - CONTEXT_LEN, STRIDE):
        ...
        if remaining <= WINDOW_FILTER:
            continue   # exclude deeply post-onset windows
...
# ── Build context_df: target = global mean (constant), covariates only ────────
print("\nBuilding context_df (no mCherry history) ...", flush=True)
ctx_rows = []
for w in windows:
    for t_idx in range(CONTEXT_LEN):
        row = {"item_id":   w["item_id"],
               "timestamp": w["ts_ctx"][t_idx],
               "target":    global_mean_mcherry}   # constant — no mCherry signal
...
with open(OUT_DIR / "tabicl_forecaster_nomcherry_h010.pkl", "wb") as f:
    pickle.dump(forecaster, f)
print("Saved tabicl_forecaster_nomcherry_h010.pkl", flush=True)
...
np.savez_compressed(OUT_DIR / "trajectories_nomcherry_h010.npz", **traj_dict)
print("Saved trajectories_nomcherry_h010.npz", flush=True)

results_df = pd.DataFrame(records)
results_df.to_csv(OUT_DIR / "predictions_nomcherry_h010.csv", index=False)
print(f"Saved predictions_nomcherry_h010.csv ({len(results_df):,} rows)", flush=True)
...
pd.DataFrame(eval_rows).to_csv(OUT_DIR / "eval_nomcherry_h010.csv", index=False)
print("Saved eval_nomcherry_h010.csv", flush=True)

# ── Onset detection evaluation ────────────────────────────────────────────────
RED_THRESHOLD = 2.302
print(f"\n=== Onset detection (threshold={RED_THRESHOLD}) ===", flush=True)
results_df["actual_list"] = results_df["actual_traj"].apply(json.loads)
results_df["pred_list"]   = results_df["pred_traj"].apply(json.loads)
results_df["actual_max"]  = results_df["actual_list"].apply(max)
results_df["is_crossing"] = results_df["actual_max"] > RED_THRESHOLD

crossing     = results_df[results_df["is_crossing"]].copy()
non_crossing = results_df[~results_df["is_crossing"] & (results_df["remaining_min"] >= 0)].copy()

crossing["pred_onset"]     = crossing["pred_list"].apply(
    lambda p: any(v > RED_THRESHOLD for v in p))
non_crossing["pred_onset"] = non_crossing["pred_list"].apply(
    lambda p: any(v > RED_THRESHOLD for v in p))

sensitivity = crossing["pred_onset"].mean()
specificity = 1 - non_crossing["pred_onset"].mean()
print(f"  Crossing windows:        {len(crossing):,}", flush=True)
print(f"  Non-crossing pre-onset:  {len(non_crossing):,}", flush=True)
print(f"  Sensitivity: {sensitivity:.3f}  Specificity: {specificity:.3f}", flush=True)
for g in ["early", "medium", "late"]:
    sc = crossing[crossing["group"] == g]
    sn = non_crossing[non_crossing["group"] == g]
    if len(sc) > 0:
        print(f"    {g:8s}  sens={sc['pred_onset'].mean():.3f}  "
              f"spec={1-sn['pred_onset'].mean():.3f if len(sn)>0 else 'n/a'}", flush=True)
...
pdf_path = OUT_DIR / "windows_nomcherry_h010_100examples.pdf"
with PdfPages(pdf_path) as pdf:
    for page in range(n_pages):
        keys_page = chosen[page * PER_PAGE: (page + 1) * PER_PAGE]
        fig, axes = plt.subplots(n_rows, n_cols, figsize=(15, 11))
        fig.suptitle(
            f"H=10 no-mCherry context — page {page+1}/{n_pages}\n"
            "grey=actual context  solid=predicted  dashed=actual future  red dashed=onset\n"
            "(model had NO access to mCherry history — only GFP/BFP/morphology)",
        ...
        for ax_idx, ax in enumerate(axes.flat):
            ...
            ax.axhline(RED_THRESHOLD, color="red", lw=0.6, linestyle=":", alpha=0.4)
